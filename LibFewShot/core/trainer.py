...
class Trainer(object):
    ...
    def train_loop(self, rank):
        ...
        for epoch_idx in range(self.from_epoch + 1, self.config["epoch"]):
            ...
            if ((self.config.get('cross_domain'))):
                train_acc = self._train_md(epoch_idx)
            else:
            ...
            if ((epoch_idx + 1) % self.val_per_epoch) == 0:
                ...
                if self.config.get('cross_domain'):
                    val_acc = self._validate_md(epoch_idx, is_test=False)
                else:
            ...
            cd = 0 if ((self.config.get('cross_domain') == None) or (self.config.get('cross_domain') == False)) else 1
            if (((epoch_idx - self.best_epoch) > self.early_stopping_thresh) and (((epoch_idx + 1) * self.config['train_episode']) >= cd * 60000)):
        ...
        return self.result_path, self.storage_path
    ...
    def _train_md(self, epoch_idx):
        ...
        for batch_idx in range(self.config['train_episode']):
            ...
            # sleep(5)
            # if self.model_type != ModelType.FINETUNING:
            #     while (batch[0].size(0) != (self.config['way_num'] * (self.config['shot_num'] + self.config['query_num']))):
            #         print("entrou uma vez")
            #         batch = next(self.train_loader)
    ...
    def _validate_md(self, epoch_idx, is_test=False):
        """
        The val/test stage.
    ...
    def _init_files(self, config):
        ...
        if self.config["resume"]:
            ...
            storage_path = self._create_intermidiate_dirs()
            # result_path = os.path.join(config["result_root"], result_dir)
            result_path = os.path.join(storage_path, result_dir)
            print(result_path)
        ...
        return result_path, log_path, checkpoints_path, viz_path, storage_path
    ...
    def _init_dataloader(self, config, cross_domain=False):
        ...
        if cross_domain:
            ...
            episodes_config_test["min_ways"] = self.config.get('min_way_test') if self.config.get('min_way_test') else self.config['test_way']
            episodes_config_test["max_ways"] = self.config.get('max_way_test') if self.config.get('max_way_test') else self.config['test_way']
            episodes_config_test["k_shot"] = self.config['test_shot']
            episodes_config_test["min_shots"] = self.config.get('min_shot_test') if self.config.get('min_shot_test') else self.config['test_shot']
            episodes_config_test["max_shots"] = self.config.get('max_shot_test') if self.config.get('max_shot_test') else self.config['test_shot']
            ...
            if self.config['classifier']['name'] in ['Baseline', 'BaselinePlus', 'BaselinePlusCV']:
                print(f"Train datasets: {self.config['train_datasets']}")
                train_dataset = MetaAlbumDataset(self.config['train_datasets'], self.config['data_root'])
                self.config['classifier']['kwargs']['num_class'] = len(train_dataset.idx_per_label)
                train_loader = iter(self._cycle(DataLoader(dataset=train_dataset, batch_size=self.config["batch_size"], shuffle=True, num_workers=self.config['workers'])))
            else:
                print(f"Train datasets: {self.config['train_datasets']}")
            ...
            print(f"Validation datasets: {self.config['val_datasets']}")
