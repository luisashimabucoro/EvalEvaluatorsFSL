...
def get_permutated_shuffle_combinations(n_way, k_shot):
    support_set_idx = np.arange(n_way*k_shot)
    class_idx_chunks = np.array_split(support_set_idx, n_way)

    shuffle_list = []
    n_splits = 30
    while n_splits != 0:
        split = []
        for chunk in class_idx_chunks:
            split.append(np.random.choice(chunk, 1))
        if split not in shuffle_list:
            shuffle_list.append(split)
            n_splits -= 1

    return support_set_idx, shuffle_list
...
def accuracy_cv(output, target, topk=1):
    """
    Calc the acc of tpok.
